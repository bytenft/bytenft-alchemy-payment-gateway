jQuery(function ($) {
	...
	const loaderUrl = bytenftalchemy_params.bytenftalchemy_loader ? encodeURI(bytenftalchemy_params.bytenftalchemy_loader) : '';
	$('body').append(`
		<div class="bytenftalchemy-loader-background"></div>
		<div class="bytenftalchemy-loader"><img src="${loaderUrl}" alt="Loading..." /></div>
	`);

	function isOurGatewaySelected() {
		return $('input[name="payment_method"]:checked').val() === bytenftalchemy_params.payment_method;
	}

	function byteNFTmarkCheckoutFormIfNeeded() {
		const $form = $("form.checkout");
		const selectedMethod = $form.find('input[name="payment_method"]:checked').val();
		const expectedId = bytenftalchemy_params.payment_method + '-checkout-form';

		if (selectedMethod === bytenftalchemy_params.payment_method) {
	...
	function byteNFTbindCheckoutHandler() {
		const formId = '#' + bytenftalchemy_params.payment_method + '-checkout-form';
		const $form = $(formId);

		// Prevent double-binding
		if ($form.data('bytenftalchemy-bound')) return;

		// Remove ALL submit handlers (from WooCommerce and others)
		$form.off("submit");

		// Bind ONLY our handler
		$form.on("submit.bytenftalchemy", function (e) {
			const isOurMethod = $(this).find('input[name="payment_method"]:checked').val() === bytenftalchemy_params.payment_method;
		...
		$form.data('bytenftalchemy-bound', true);
	}

	function showLoader() {
		$(".bytenftalchemy-loader, .bytenftalchemy-loader-background").show();
	}

	function hideLoader() {
		$(".bytenftalchemy-loader, .bytenftalchemy-loader-background").hide();
	}
	...
	function handleFormSubmit(e) {
		...
		if (selectedPaymentMethod !== bytenftalchemy_params.payment_method) {
			isSubmitting = false;
			return true;
		}
		...
		$('.bytenftalchemy-loader-background, .bytenftalchemy-loader').show();
	...
	function openPopup(paymentLink, customerEmail) {
		...
		$('#bytenftalchemy-manual-link').attr('href', paymentLink);
		$('#bytenftalchemy-qr-img').attr('src', "https://image-charts.com/chart?chs=95x95&cht=qr&chl=" + paymentLink + "&choe=UTF-8");

		if (customerEmail) {
			$('#bytenftalchemy-payment-popup .email-info .bytenftalchemy-customer-email').text(customerEmail);
		}

		// Reset UI
		$('#bytenftalchemy-payment-popup .payment-link-div').show();
		$('#bytenftalchemy-payment-popup .payment-timeline, .tabs-wrapper, .thank-you-msg, .failed-msg, .payment-failed').hide();
		$('#bytenftalchemy-payment-popup').fadeIn();

		startPolling(() => {
			$('#bytenftalchemy-payment-popup').fadeOut();
		});

		$('#bytenftalchemy-cancel-order').off('click').on('click', function () {
			$('#bytenftalchemy-payment-popup').fadeOut();
			stopPolling();
			resetButton();
		});
	...
	function startPolling(onComplete) {
		...
		pollingInterval = setInterval(() => {
			$.ajax({
				type: 'POST',
				url: bytenftalchemy_params.ajax_url,
				data: {
					action: 'check_payment_status',
					order_id: orderId,
					security: bytenftalchemy_params.bytenftalchemy_nonce,
				},
				dataType: 'json',
				success: (statusResponse) => {
					...
					const popup = $("#bytenftalchemy-payment-popup");
	...
	$(document).on('click', '#bytenftalchemy-payment-popup .switch_tab', function () {
		const tab = $(this).data('tab');

		$('#bytenftalchemy-payment-popup .switch_tab').removeClass('active');
	...
	$(document).on('click', '#bytenftalchemy-close-payment-popup', function () {
		$('#bytenftalchemy-payment-popup').fadeOut();
		stopPolling();
		resetButton();
	});

	$('#bytenftalchemy-send-link-btn').on('click', function () {
		const email = $('#bytenftalchemy-payment-popup input[name=email]').val().trim();
		const phone = $('#bytenftalchemy-payment-popup input[name=phone]').val().trim();
		...
		$.ajax({
			...
			beforeSend: function () {
				$('#bytenftalchemy-send-link-btn').text('Sending...');
			},
			success: function (res) {
				const $msg = $('#bytenftalchemy-send-link-msg');
				$msg.remove();
				...
				$('#bytenftalchemy-send-link-btn')
				.after(`<div id="bytenftalchemy-send-link-msg" style="margin-top:8px;color:${color};font-size:14px;">${message}</div>`);
				
				setTimeout(() => {
					$(document).find('#bytenftalchemy-send-link-msg').remove();
				}, 3000);
			},
			error: function () {
			...
			complete: function () {
				$('#bytenftalchemy-send-link-btn').text('Send');
			}
		});
	});
