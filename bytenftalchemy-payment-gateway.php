 ...
 * Text Domain: bytenftalchemy-payment-gateway
 * Plugin URI: 
 * Version: 1.0.7
...
define('BYTENFTALCHEMY_PAYMENT_GATEWAY_MIN_PHP_VER', '8.0');
define('BYTENFTALCHEMY_PAYMENT_GATEWAY_MIN_WC_VER', '6.5.4');
define('BYTENFTALCHEMY_PAYMENT_GATEWAY_FILE', __FILE__);
define('BYTENFTALCHEMY_PAYMENT_GATEWAY_PLUGIN_DIR', plugin_dir_path(__FILE__));

// Include utility functions
require_once BYTENFTALCHEMY_PAYMENT_GATEWAY_PLUGIN_DIR . 'includes/bytenftalchemy-payment-gateway-utils.php';
...
spl_autoload_register(function ($class) {
	if (strpos($class, 'BYTENFTALCHEMY_PAYMENT_GATEWAY') === 0) {
		$class_file = BYTENFTALCHEMY_PAYMENT_GATEWAY_PLUGIN_DIR . 'includes/class-' . str_replace('_', '-', strtolower($class)) . '.php';
...
BYTENFTALCHEMY_PAYMENT_GATEWAY_Loader::get_instance();

add_action('woocommerce_cancel_unpaid_order', 'bytenftalchemy_cancel_unpaid_order_action');
add_action('woocommerce_order_status_cancelled', 'bytenftalchemy_cancel_unpaid_order_action');
 ...
 */
function bytenftalchemy_cancel_unpaid_order_action($order_id)
{
	global $wpdb;
	...
	if ($order->has_status('pending')) {
		...
		wp_cache_delete('bytenftalchemy_payment_link_uuid_' . $order_id, 'bytenftalchemy_payment_gateway');
		wp_cache_delete('bytenftalchemy_payment_row_' . $order_id, 'bytenftalchemy_payment_gateway'); // Clear row cache
	...
	$cache_key    = 'bytenftalchemy_payment_row_' . $order_id;
	...
	$url      = BYTENFTALCHEMY_BASE_URL . $apiPath;
